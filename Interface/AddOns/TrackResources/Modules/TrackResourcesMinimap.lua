...
function TrackResourcesMinimap:AddIcon(itemData, playerWorldPosition)
    ...
    icon.texture:SetTexture(textureID, "CLAMPTOBLACKADDITIVE", nil, TrackResourcesCfg.settings.filterMode)
...
function TrackResourcesMinimap:ReloadIcons(force)
    -- Debug("Reloading Minimap Icons")
    if not TrackResourcesCfg.settings.showMinimap then return end
    local currentUiMapID = C_Map.GetBestMapForUnit("player")
    if not force and currentUiMapID == lastUiMapID then return end
