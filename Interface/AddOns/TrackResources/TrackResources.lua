...
local FIND_MINERALS = 2580
local FIND_HERBS = 2383
...
function TrackResources:OnLoad()
    ...
    TrackResourcesCfg.settings.filterMode =  TrackResourcesCfg.settings.filterMode or "TRILINEAR"
    TrackResourcesCfg.settings.useClassicCodex = TrackResourcesCfg.settings.useClassicCodex or false
    TrackResourcesCfg.settings.alpha = TrackResourcesCfg.settings.alpha or TR.iconSettings.alpha
...
function TrackResources:PLAYER_ALIVE()
    if ohNoDead and not UnitIsDeadOrGhost("player") then
        TrackResources:PLAYER_UNGHOST()
    end
end

function TrackResources:PLAYER_UNGHOST()
    ohNoDead = false
    if TR.isHerbalist or TR.isMiner then
        if TR.isHerbalist and TR.isMiner then
            self:Important("Can't have tracking for both resources at once, defaulting to finding herbs.")
        else
            self:Important("Turning on resource tracking again.")
        end
        if TR.isHerbalist then
            CastSpellByID(FIND_HERBS)
        else
            CastSpellByID(FIND_MINERALS)
        end
    end
end
...
function TrackResources:CheckProfessionSpells()
    TR.isHerbalist = TrackResourcesCfg.settings.alwaysShow or DoesSpellExist(select(1, GetSpellInfo(FIND_HERBS)))
    TR.isMiner = TrackResourcesCfg.settings.alwaysShow or DoesSpellExist(select(1, GetSpellInfo(FIND_MINERALS)))
end
...
function TrackResources:AddResourceToDB(itemID, textureID, uiMapID, x, y)
    ...
    if not TrackResourcesCfg.db[uiMapID][itemID][posKey] then
        self:Print("Added %s (%s) (%0.2f, %0.2f)", TR.cache.items[itemID].itemName,  GetRealZoneText(), x*100, y*100);
        TrackResourcesCfg.db[uiMapID][itemID][posKey] = { itemID, textureID , uiMapID, x, y }
        TrackResourcesMap:AddIcon(uiMapID, posKey, TrackResourcesCfg.db[uiMapID][itemID][posKey])
...
function TrackResources:ParseCommands(text, chatBoxFrame)
    ...
    local commands = {
        ...
        alpha = function(alpha)
            TrackResourcesCfg.settings.alpha = tonumber(alpha)
            TrackResourcesMap:UpdateIcons()
        end,
        prof = function()
            ...
            if TrackResourcesCfg.settings.alwaysShow then
                ...
                TR.isHerbalist = DoesSpellExist(select(1, GetSpellInfo(FIND_HERBS)))
                TR.isMiner = DoesSpellExist(select(1, GetSpellInfo(FIND_MINERALS)))
        ...
        filtermode = function(filterMode)
            filterMode = filterMode:upper()
            if filterMode == "LINEAR" or filterMode == "TRILINEAR" or filterMode == "NEAREST" then
                self:Print("Using texture filterMode %s", filterMode)
                TrackResourcesCfg.settings.filterMode = filterMode
                TrackResourcesMap:ResetIconCache()
                TrackResourcesMinimap:ReloadIcons(true)
            else
                self:PrintHelp()
            end
        end,
        codex = function()
            TrackResourcesCfg.settings.useClassicCodex = not TrackResourcesCfg.settings.useClassicCodex
            if TrackResourcesCfg.settings.useClassicCodex then
                self:Print("Using ClassicCodex Database. Warning: there's a lot of data..")
            else
                self:Print("Disabling the use of ClassicCodex Database")
            end
            TrackResourcesMap:ResetIconCache()
        end,
    }
...
function TrackResources:PrintHelp()
    ...
    help("alpha n.n (alpha/transparency level, from 0.0 to 1.0, default 0.75)")
    help("prof (toggles showing all profession resources, or currently learned professions)")
    if CodexDB and CodexDB.objects then
        help("codex (beta: toggles using ClassicCodex database on world map)")
    end
end